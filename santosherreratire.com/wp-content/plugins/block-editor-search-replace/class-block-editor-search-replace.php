...
Plugin URI: https://krasenslavov.com/plugins/block-editor-search-replace/
Description: Quickly search and replace any text inside the Block Editor content area; Classic Editor support incliuded.
...
Copyright 2018-2023 Krasen Slavov (email: hello@krasenslavov.com)
...
if ( ! class_exists( 'Block_Editor_Search_Replace' ) ) {
	...
	class Block_Editor_Search_Replace {
		...
		const UUID             = 'besr';
		const TEXTDOMAIN       = 'block-editor-search-replace';
		const PLUGIN_NAME      = 'Block Editor Search & Replace';
		const PLUGIN_DOCURL    = 'https://krasenslavov.com/plugins/block-editor-search-replace/';
		const PLUGIN_WPORGURL  = 'https://wordpress.org/support/plugin/block-editor-search-replace/';
		const PLUGIN_WPORGRATE = 'https://wordpress.org/support/plugin/block-editor-search-replace/reviews/?filter=5';
			...
			if ( ! get_option( 'besr_rating_notice' ) ) {
				?>
					<div class="notice notice-success is-dismissible">
						<h3>Block Editor Search & Replace</h3>
						<p>
						...
						<p>
							...
							<a href="?besr_rating_notice_dismiss" class="button"><strong>I already did</strong></a>
							<a href="?besr_rating_notice_dismiss" class="button"><strong>Don't show this notice again!</strong></a>
						</p>
					</div>
				<?php
			}
		...
		public function rating_notice_dismiss() {
			if ( isset( $_GET['besr_rating_notice_dismiss'] ) ) {
				add_option( 'besr_rating_notice', 1 );
			}
		}
	...
	new Block_Editor_Search_Replace();

	// Core
	require_once 'classes/core/class-besr-options.php';

	// Init
	require_once 'classes/class-besr-init.php';

	// Events
	require_once 'classes/events/class-besr-search-replace.php';
}
